...
export default function Footer({ dict, brandName }: FooterProps) {
  return (
    <footer className="border-t border-[var(--border)] bg-[var(--surface)]/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {/* Quick links */}
          <div>
            <h4 className="text-sm font-semibold mb-3 text-[var(--foreground)]/70">
              快速链接
            </h4>
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(dict.nav).map(([key, label]) => (
                <Link
                  key={key}
                  href={key === 'home' ? '/' : `/${key}`}
                  className="text-sm text-[var(--foreground)]/40 hover:text-[var(--foreground)] transition-colors"
                >
                  {label}
                </Link>
              ))}
