    ...
    <div className="pt-24 pb-20 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {services.map((svc) => {
            ...
            return (
              <div
                key={svc.id}
                className="p-8 rounded-2xl border border-[var(--border)] bg-[var(--surface)]/50 hover:bg-[var(--surface)] hover:border-[var(--primary)]/30 transition-all"
              >
                ...
                <ul className="space-y-2">
                  {svc.features.map((f, i) => (
                    <li
                      key={i}
                      className="text-sm text-[var(--foreground)]/60 flex items-center gap-2"
                    >
        ...
        <div className="text-center mt-12">
          <Link
            href={`/${lang}/contact`}
            className="inline-flex px-8 py-3 rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--accent)] text-white font-medium hover:opacity-90 transition-opacity"
          >
