...
import { Activity, FileText, Clock, RefreshCw } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/status').then(r => r.json()).then(d => setStatus(d.status || 'unknown')).catch(() => {})
  }, [])
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface)]/50">
          ...
          <span className={`text-lg font-semibold ${status === 'success' ? 'text-green-400' : 'text-yellow-400'}`}>{status === 'success' ? '✅ 在线' : status || '加载中...'}</span>
        </div>
        <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface)]/50">
          <div className="flex items-center gap-2 mb-1"><FileText size={16} className="text-[var(--accent)]" /><span className="text-sm text-[var(--foreground)]/50">编辑模块</span></div>
          <span className="text-lg font-semibold">8 个</span>
        </div>
        <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface)]/50">
      ...
      <h2 className="text-lg font-semibold mb-3 flex items-center gap-2"><Clock size={16} />最近修改</h2>
